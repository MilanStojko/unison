                    ...
                    <div class="card-content">
                        <form action="{{ route('admin.users.update', auth()->user())}}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="tel" name="cellphone" class="form-control @error('cellphone') is-invalid @enderror" placeholder="">
                            </div>
                            {{-- INDIRIZZO --}}
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check d-inline-block m-3">
                                    <input type="checkbox" class="form-check-input" name="categories[]" value="{{$category->id}}" id="{{$category->slug}}"
                                    @if($errors->any())
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check d-inline-block m-3">
                                    <input type="checkbox" class="form-check-input" name="availabilities[]" value="{{$availability->id}}" id="{{$availability->slug}}"
                                    @if($errors->any())
                                        {{in_array($availability->id, old('availabilities', [])) ? "checked": ""}}
                                    @else
                                        {{$user->availabilities->contains($availability) ? 'checked' : ''}}>
                                    @endif
                                        <label for="{{$availability->slug}}">{{$availability->name}}</label>
                                    </div>
                                    @endforeach
